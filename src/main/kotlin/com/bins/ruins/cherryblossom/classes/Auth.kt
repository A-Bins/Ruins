                        ...
                        authMsg.edit {
                            embed {
                                if (count == 0) {
                                    ...
                                    footer = EmbedBuilder.Footer().apply {
                                        text =
                                            "${Bukkit.getOfflinePlayer(completers[msg.author!!.id.value.toLong()]!!).name} | ${completers[msg.author!!.id.value.toLong()]}"
                                        icon =
                                            "https://crafatar.com/avatars/${completers[msg.author!!.id.value.toLong()]!!}?size=25`"
                                    }
                                    requests.remove(authCode)
                                }
                            }
